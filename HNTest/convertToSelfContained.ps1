...
function Convert-CSharpProject {
    ...
    try {
        ...
        if (-not $propertyGroup) {
            # Create a new PropertyGroup if none exists with proper namespace
            $namespaceUri = $projectXml.DocumentElement.NamespaceURI
            $propertyGroup = $projectXml.CreateElement("PropertyGroup", $namespaceUri)
        ...
        if ($existingProperty) {
            ...
            # Add WindowsAppSDKSelfContained property with proper namespace
            $namespaceUri = $projectXml.DocumentElement.NamespaceURI
            $selfContainedElement = $projectXml.CreateElement("WindowsAppSDKSelfContained", $namespaceUri)
